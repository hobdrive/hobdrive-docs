...
### Важные особенности поведения iOS

- **Телефон должен быть разблокирован**: iOS запускает приложение только если телефон разблокирован
- **CarPlay и заблокированный телефон**: iOS может запустить приложение при включенном CarPlay, даже если телефон заблокирован
- **Подключение к Bluetooth/ELM**: Соединение с Bluetooth/ELM адаптером начинается только если экран телефона активен

Эти ограничения являются частью функций безопасности и конфиденциальности iOS. Для наилучшей работы убедитесь, что телефон разблокирован при подключении к CarPlay.
