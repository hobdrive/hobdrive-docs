...
Это руководство теперь играет роль **обзорного уровня**: дает базовый сценарий настройки и понимание экранов.

Детальные технические материалы вынесены в отдельные статьи базы знаний:

- Подключение, профили, совместимость: [Готовые профили автомобилей](profiles.md), [Проблемы кривых адаптеров ELM327](bad-elms.md), [Диагностика ABS/SRS и отдельных ЭБУ](custom-ecus.md).
- Расход и калибровка: [Методы расчета расхода и коэффициенты](fuel-methods.md), [Расход топлива: вопросы и ответы](fuel-questions.md), [Разница в показаниях скорости](speed-diff.md), [Коррекция показаний напряжения](voltage.md).
- Экраны и настройка интерфейса: [Настройка главных экранов](screen-settings.md), [Оформление, скины и расположение датчиков](layouts.md), [Спецификация языка разметки](LAYOUT_SPEC_RU.md), [Динамические выражения](dynamic-expr.md), [Базовый синтаксис выражений](dynamic-expr-core.md).
- Данные, отчеты, поддержка: [Бэкап и перенос настроек](backups.md), [Статистика поездок](statistics.md), [Как отправить отчет о проблеме](problems.md), [Лицензирование](licensing.md).
- iOS/CarPlay: [Автоматический запуск через Shortcuts](carplay-shortcut.md).

Если вы настраиваете HobDrive впервые, пройдите этот мануал сверху вниз, а в углубленные статьи переходите по ссылкам в соответствующих разделах.
...
Подробнее по качеству и совместимости адаптеров: [Проблемы кривых адаптеров ELM327](bad-elms.md).
...
Подробнее: [Готовые профили автомобилей](profiles.md), [Диагностика ABS/SRS и отдельных ЭБУ](custom-ecus.md).
...
Подробная статья по профилям и шаблонам: [Готовые профили автомобилей](profiles.md).
...
Подробные алгоритмы и примеры выбора метода: [Методы расчета расхода и их калибровочные коэффициенты](fuel-methods.md), [Расход топлива: основные принципы](fuel-questions.md).
...
Отдельно по коррекции скорости/пробега: [Разница в показаниях скорости](speed-diff.md).
...
Подробно о порядке экранов, закреплении, скрытии, персональных параметрах и пользовательском экране: [Настройка главных экранов](screen-settings.md).
...
Расширенная настройка через файлы раскладок: [Оформление, скины и расположение датчиков](layouts.md), [Спецификация языка разметки](LAYOUT_SPEC_RU.md), [Динамические выражения](dynamic-expr.md).
...
Подробно о файлах, переносе и восстановлении: [Как перенести настройки или сделать бэкап](backups.md).

Подробно по облачной/локальной статистике: [Статистика поездок и записей](statistics.md).
...
Подробные материалы для продвинутой кастомизации:

- [Оформление, скины и расположение датчиков](layouts.md)
- [Спецификация языка разметки HobDrive](LAYOUT_SPEC_RU.md)
- [Динамические выражения в файлах настроек](dynamic-expr.md)
- [Динамические выражения: базовый синтаксис](dynamic-expr-core.md)
- [Спецификация формата ECUXML](ecuxml_spec.md)
...
Пошаговая инструкция для автозапуска на CarPlay: [Автоматический запуск через Shortcuts](carplay-shortcut.md).
...
В этом мануале оставляем только обзорный алгоритм. Полная техническая часть с формулами, AFR/VE, коэффициентами MAP/MAF/Injector/EngineLoad и примерами расчета вынесена в отдельные материалы:

- [Методы расчета расхода и их калибровочные коэффициенты](fuel-methods.md)
- [Расход топлива: основные принципы и типовые вопросы](fuel-questions.md)
- [Разница в показаниях скорости](speed-diff.md)
...
Детальный разбор некачественных адаптеров: [Проблемы кривых адаптеров ELM327](bad-elms.md).
...
Подробнее: [Методы расчета расхода](fuel-methods.md), [Расход топлива: вопросы и ответы](fuel-questions.md), [Разница в показаниях скорости](speed-diff.md).
...
Если проблема воспроизводится, отправьте отчет: [Отчет о проблеме, сбое](problems.md).
